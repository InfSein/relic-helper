...
export const getItemInfo = (item: `${number}` | number | CalculatedItem) => {
  ...
  // * 组装肝武信息
  const relicItemMap = getRelicItemMap()
  if (relicItemMap[itemID]) {
    itemInfo.relicInfo = relicItemMap[itemID]
  }
...
import { getRelicItemMap, type RelicItemInfo } from './relic'
export const getItemContexts = (
  ...
  const itemLanguage = 'zh' as "zh" | "ja" | "en"
  const options = [
    {
      label: '复制物品名',
      key: 'copy-item-name',
