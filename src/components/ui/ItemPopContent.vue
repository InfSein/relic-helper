...
interface ItemPopContentProps {
  /** 道具信息 */
  itemInfo: ItemInfo
  /** 物品按钮所处容器的ID，在模态框等场景时必须传递，否则无法正常复制物品名 */
  containerId?: string
}
const props = defineProps<ItemPopContentProps>()
  ...
  .item-descriptions {
    ...
    .description-block {
      ...
      .title {
        ...
        --size-small: calc(var(--n-font-size, 14px) - 2px);
        --textgap-left: calc(var(--n-font-size, 14px) - 1px);
